...
class Person < ApplicationRecord # rubocop:disable Metrics/ClassLength
  ...
  belongs_to :ministry
  belongs_to :spouse, foreign_key: :spouse_id, class_name: "Person", inverse_of: :spouse, autosave: true, optional: true
  ...
  class << self
    ...
    def create_or_update_assignments_from_entity(entity)
      Array.wrap(entity["ministry:relationship"]).select { |r| r["ministry_of_service"] }.map { |relationship|
